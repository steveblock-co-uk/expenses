...
/*
group([new Transaction(0, null, 'aaa'), new Transaction(1, null, 'babb')], function(x) { return x.getDescription(); }, function(x) { return x; });

function hashIntegerSet(x) {
  return x.slice().sort().join(':');
}

{
  // Get maximum substring for every pair of items. Map from ID hash to object.
  var idHashToSubstring = {};
  for (var i1 in items) {
    var id1 = items[i1].getId();
    var string1 = getter(items[i1]);
    for (var i2 in items) {
      if (i1 === i2) {
        continue;
      }
      var id2 = items[i2].getId();
      var string2 = getter(items[i2]);
      var idHash = hashIntegerSet([id1, id2]);
      var substring = getLongestCommonSubstring(string1, string2).trim();
      if (idHashToSubstring[idHash] === undefined) {
        idHashToSubstring[idHash] = {
          id1: id1,
          id2: id2,
          substring: substring,
        };
  ...
  // Get map from substring to set of IDs. Note that each ID may appear multiple times, as it may match to multiple IDs
  // by different substrings.
  Object.keys(idHashToSubstring).forEach(function(idHash) {
    var o = idHashToSubstring[idHash];
    if (substringToItemIds[o.substring] === undefined) {
      substringToItemIds[o.substring] = [];
    }
    substringToItemIds[o.substring].push(o.id1);
    substringToItemIds[o.substring].push(o.id2);
  });

  // Determine minimum heuristic for a match
  // TODO
  var minimumHeuristic = 10;

  // Form groups. Map from item ID to group ID.
  var groups = {};
  var nextGroupId = 0;
  for (var idHash in substrings) {
    var o = substrings[idHash];
    if (heuristic(o.substring) < minimumHeuristic) {
      continue;
    }
    console.log(o);
    var groupId1 = groups[o.id1];
    var groupId2 = groups[o.id2];
    if (groupId1 === undefined && groupId2 === undefined) {
      groups[o.id1] = nextGroupId;
      groups[o.id2] = nextGroupId;
      ++nextGroupId;
    } else if (groupId1 != null) {
      groups[o.id2] = groupId1;
    } else if (groupId2 != null) {
      groups[o.id1] = groupId2;
    }
  }
  for (var i in items) {
...
function getMinimumLengthString(strings) {
  var minimumLength = Infinity;
  var minimumLengthIndex;
  for (var i in strings) {
    var string = strings[i];
    if (string.length < minimumLength) {
      minimumLength = string.length;
      minimumLengthIndex = i;
    }
  }
  return strings[minimumLengthIndex];
}
...
function generateAllSubstrings(string) {
  ...
  for (var i = 0; i < string.length; ++i) {
    for (var j = i + 1; j <= string.length; ++j) {
      strings.push(string.substr(i, j));
    }
  }
  return strings;
}

// Gets a map from substring to a list of items. stringGetter gets the string from an item.
function getCommonSubstrings(items, stringGetter) {
  // Arranging all permutations of N strings of length L is O(L^(N-1)). Searching for a common substring is O(L), giving
  // O(L^N) overall. Instead, we form all potential substrings in time O(L^2) then search all N strings, each in time
  // O(L^2), giving O(NL^4) overall.

  var strings = items.map(stringGetter);
  var minimumLengthString = getMinimumLengthString(strings);
  var searchSubstrings = generateAllSubstrings(minimumLengthString);

  var substrings = {};
  searchSubstrings.forEach(function(searchSubstring) {
    var allStringsMatch = strings.every(function(string) {
      return string.search(searchSubstring) != -1;
    });
    if (allStringsMatch) {
      substrings[searchSubstring] = searchSubstring;
    }
  });
  console.log(substrings);
}

//getCommonSubstrings([{s: 'abcdyuiepfgh'}, {s: 'yuip'}], function(x) { return x.s; });
